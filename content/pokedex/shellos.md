---
title: 'Shellos'
subtitle: 'Its colors and shapes differ from region to region. In the Sinnoh region, two types are confirmed.'
date: 2026-04-28
dex_number: '422'
skins: ['Shiny']
forms:
  - name: 'West Sea'
    kind: 'form'
    types: ['Water']
    abilities: ['Sticky Hold', 'Storm Drain']
    hidden_ability: 'Sand Force'
    sprite: 'images/pokedex/shellos.png'
    note: 'The pink-bodied West Sea variant. Identical stats and types to the East Sea form; only the appearance differs by region of origin.'
    egg_groups: ['amorphous', 'water 1']
    egg_steps: 5140
    growth_rate: 'medium_fast'
    height_m: 0.3
    weight_kg: 6.3
    base_stats:
      hp: 76
      atk: 48
      def: 48
      spa: 57
      spd: 62
      spe: 34
    locations: ['Shiloh: Crystal View', 'Zeinova: Baremaw']
    competitive_sets:
      - name: 'Defensive'
        tier: 'LC'
        moves:
          - 'Scald'
          - 'Recover'
          - ['Earth Power', 'Ice Beam']
          - ['Toxic', 'Clear Smog', 'Yawn']
        item: 'Eviolite'
        nature: 'Bold'
        evs:
          hp: 228
          def: 132
          spd: 100
          spe: 4
      - name: 'Curse Sweeper'
        tier: 'LC'
        moves:
          - 'Facade'
          - 'Curse'
          - 'Amnesia'
          - 'Recover'
        item: 'Eviolite'
        nature: 'Impish'
        evs:
          hp: 228
          def: 132
          spd: 100
          spe: 4
    notable_moves:
      - move: 'Mud-Slap'
        source: 'Level 1'
        type: 'Ground'
        category: 'Special'
        power: '20'
        accuracy: '100'
        pp: '10'
      - move: 'Mud Sport'
        source: 'Level 2'
        type: 'Ground'
        category: 'Status'
        power: '—'
        accuracy: '∞'
        pp: '15'
      - move: 'Harden'
        source: 'Level 4'
        type: 'Normal'
        category: 'Status'
        power: '—'
        accuracy: '∞'
        pp: '30'
      - move: 'Water Pulse'
        source: 'Level 7'
        type: 'Water'
        category: 'Special'
        power: '60'
        accuracy: '100'
        pp: '20'
      - move: 'Mud Bomb'
        source: 'Level 11'
        type: 'Ground'
        category: 'Special'
        power: '65'
        accuracy: '85'
        pp: '10'
      - move: 'Hidden Power'
        source: 'Level 16'
        type: 'Normal'
        category: 'Special'
        power: '60'
        accuracy: '100'
        pp: '15'
      - move: 'Rain Dance'
        source: 'Level 22'
        type: 'Water'
        category: 'Status'
        power: '—'
        accuracy: '∞'
        pp: '5'
      - move: 'Body Slam'
        source: 'Level 29'
        type: 'Normal'
        category: 'Physical'
        power: '85'
        accuracy: '100'
        pp: '15'
      - move: 'Muddy Water'
        source: 'Level 37'
        type: 'Water'
        category: 'Special'
        power: '90'
        accuracy: '85'
        pp: '10'
      - move: 'Recover'
        source: 'Level 46'
        type: 'Normal'
        category: 'Status'
        power: '—'
        accuracy: '∞'
        pp: '10'
    tms:
      - move: 'Attract'
        type: 'Normal'
        category: 'Status'
        power: '—'
        accuracy: '100'
        pp: '15'
      - move: 'Blizzard'
        type: 'Ice'
        category: 'Special'
        power: '110'
        accuracy: '70'
        pp: '5'
      - move: 'Confide'
        type: 'Normal'
        category: 'Status'
        power: '—'
        accuracy: '∞'
        pp: '20'
      - move: 'Dive'
        type: 'Water'
        category: 'Physical'
        power: '80'
        accuracy: '100'
        pp: '10'
      - move: 'Double Team'
        type: 'Normal'
        category: 'Status'
        power: '—'
        accuracy: '∞'
        pp: '15'
      - move: 'Facade'
        type: 'Normal'
        category: 'Physical'
        power: '70'
        accuracy: '100'
        pp: '20'
      - move: 'Frustration'
        type: 'Normal'
        category: 'Physical'
        power: '—'
        accuracy: '100'
        pp: '20'
      - move: 'Hail'
        type: 'Ice'
        category: 'Status'
        power: '—'
        accuracy: '∞'
        pp: '10'
      - move: 'Hidden Power'
        type: 'Normal'
        category: 'Special'
        power: '60'
        accuracy: '100'
        pp: '15'
      - move: 'Ice Beam'
        type: 'Ice'
        category: 'Special'
        power: '90'
        accuracy: '100'
        pp: '10'
      - move: 'Infestation'
        type: 'Bug'
        category: 'Special'
        power: '20'
        accuracy: '100'
        pp: '20'
      - move: 'Protect'
        type: 'Normal'
        category: 'Status'
        power: '—'
        accuracy: '∞'
        pp: '10'
      - move: 'Rain Dance'
        type: 'Water'
        category: 'Status'
        power: '—'
        accuracy: '∞'
        pp: '5'
      - move: 'Rest'
        type: 'Psychic'
        category: 'Status'
        power: '—'
        accuracy: '∞'
        pp: '10'
      - move: 'Return'
        type: 'Normal'
        category: 'Physical'
        power: '—'
        accuracy: '100'
        pp: '20'
      - move: 'Round'
        type: 'Normal'
        category: 'Special'
        power: '60'
        accuracy: '100'
        pp: '15'
      - move: 'Scald'
        type: 'Water'
        category: 'Special'
        power: '80'
        accuracy: '100'
        pp: '15'
      - move: 'Secret Power'
        type: 'Normal'
        category: 'Physical'
        power: '70'
        accuracy: '100'
        pp: '20'
      - move: 'Sleep Talk'
        type: 'Normal'
        category: 'Status'
        power: '—'
        accuracy: '∞'
        pp: '10'
      - move: 'Substitute'
        type: 'Normal'
        category: 'Status'
        power: '—'
        accuracy: '∞'
        pp: '10'
      - move: 'Surf'
        type: 'Water'
        category: 'Special'
        power: '90'
        accuracy: '100'
        pp: '15'
      - move: 'Swagger'
        type: 'Normal'
        category: 'Status'
        power: '—'
        accuracy: '90'
        pp: '15'
      - move: 'Toxic'
        type: 'Poison'
        category: 'Status'
        power: '—'
        accuracy: '90'
        pp: '10'
    tutor_moves:
      - move: 'Earth Power'
        type: 'Ground'
        category: 'Special'
        power: '90'
        accuracy: '100'
        pp: '10'
      - move: 'Icy Wind'
        type: 'Ice'
        category: 'Special'
        power: '55'
        accuracy: '95'
        pp: '15'
      - move: 'Pain Split'
        type: 'Normal'
        category: 'Status'
        power: '—'
        accuracy: '∞'
        pp: '20'
      - move: 'Snore'
        type: 'Normal'
        category: 'Special'
        power: '50'
        accuracy: '100'
        pp: '15'
      - move: 'Water Pulse'
        type: 'Water'
        category: 'Special'
        power: '60'
        accuracy: '100'
        pp: '20'
    egg_moves:
      - move: 'Acid Armor'
        type: 'Poison'
        category: 'Status'
        power: '—'
        accuracy: '∞'
        pp: '20'
      - move: 'Amnesia'
        type: 'Psychic'
        category: 'Status'
        power: '—'
        accuracy: '∞'
        pp: '20'
      - move: 'Brine'
        type: 'Water'
        category: 'Special'
        power: '65'
        accuracy: '100'
        pp: '10'
      - move: 'Clear Smog'
        type: 'Poison'
        category: 'Special'
        power: '50'
        accuracy: '∞'
        pp: '15'
      - move: 'Counter'
        type: 'Fighting'
        category: 'Physical'
        power: '—'
        accuracy: '100'
        pp: '20'
      - move: 'Curse'
        type: 'Ghost'
        category: 'Status'
        power: '—'
        accuracy: '∞'
        pp: '10'
      - move: 'Fissure'
        type: 'Ground'
        category: 'Physical'
        power: '—'
        accuracy: '30'
        pp: '5'
      - move: 'Memento'
        type: 'Dark'
        category: 'Status'
        power: '—'
        accuracy: '100'
        pp: '10'
      - move: 'Mirror Coat'
        type: 'Psychic'
        category: 'Special'
        power: '—'
        accuracy: '100'
        pp: '20'
      - move: 'Mist'
        type: 'Ice'
        category: 'Status'
        power: '—'
        accuracy: '∞'
        pp: '30'
      - move: 'Sludge'
        type: 'Poison'
        category: 'Special'
        power: '65'
        accuracy: '100'
        pp: '20'
      - move: 'Spit Up'
        type: 'Normal'
        category: 'Special'
        power: '—'
        accuracy: '100'
        pp: '10'
      - move: 'Stockpile'
        type: 'Normal'
        category: 'Status'
        power: '—'
        accuracy: '∞'
        pp: '20'
      - move: 'Swallow'
        type: 'Normal'
        category: 'Status'
        power: '—'
        accuracy: '∞'
        pp: '10'
      - move: 'Trump Card'
        type: 'Normal'
        category: 'Special'
        power: '—'
        accuracy: '∞'
        pp: '5'
      - move: 'Yawn'
        type: 'Normal'
        category: 'Status'
        power: '—'
        accuracy: '∞'
        pp: '10'
    evolution_family:
      - species: 'Shellos'
        slug: 'shellos'
        depth: 0
        method: ''
      - species: 'Gastrodon'
        slug: 'gastrodon'
        depth: 1
        method: 'Level 30'
  - name: 'East Sea'
    kind: 'form'
    types: ['Water']
    abilities: ['Sticky Hold', 'Storm Drain']
    hidden_ability: 'Sand Force'
    sprite: 'images/pokedex/shellos-east.png'
    note: 'The blue-bodied East Sea variant. Identical stats and types to the West Sea form; only the appearance differs by region of origin.'
    egg_groups: ['amorphous', 'water 1']
    egg_steps: 5140
    growth_rate: 'medium_fast'
    height_m: 0.3
    weight_kg: 6.3
    base_stats:
      hp: 76
      atk: 48
      def: 48
      spa: 57
      spd: 62
      spe: 34
    locations: ['Baremaw', 'Baywharf Cove', 'Breezelton Village', 'Bridgerun', 'Findale Harbor', 'Greenholm', 'Icy Plains', 'Mossy Stone', 'Pokémon League', 'Pure Harbor', 'Redgrove', 'Sparkmont City', 'Spiritvale']
    competitive_sets:
      - name: 'Defensive'
        tier: 'LC'
        moves:
          - 'Scald'
          - 'Recover'
          - ['Earth Power', 'Ice Beam']
          - ['Toxic', 'Clear Smog', 'Yawn']
        item: 'Eviolite'
        nature: 'Bold'
        evs:
          hp: 228
          def: 132
          spd: 100
          spe: 4
      - name: 'Curse Sweeper'
        tier: 'LC'
        moves:
          - 'Facade'
          - 'Curse'
          - 'Amnesia'
          - 'Recover'
        item: 'Eviolite'
        nature: 'Impish'
        evs:
          hp: 228
          def: 132
          spd: 100
          spe: 4
    notable_moves:
      - move: 'Mud-Slap'
        source: 'Level 1'
        type: 'Ground'
        category: 'Special'
        power: '20'
        accuracy: '100'
        pp: '10'
      - move: 'Mud Sport'
        source: 'Level 2'
        type: 'Ground'
        category: 'Status'
        power: '—'
        accuracy: '∞'
        pp: '15'
      - move: 'Harden'
        source: 'Level 4'
        type: 'Normal'
        category: 'Status'
        power: '—'
        accuracy: '∞'
        pp: '30'
      - move: 'Water Pulse'
        source: 'Level 7'
        type: 'Water'
        category: 'Special'
        power: '60'
        accuracy: '100'
        pp: '20'
      - move: 'Mud Bomb'
        source: 'Level 11'
        type: 'Ground'
        category: 'Special'
        power: '65'
        accuracy: '85'
        pp: '10'
      - move: 'Hidden Power'
        source: 'Level 16'
        type: 'Normal'
        category: 'Special'
        power: '60'
        accuracy: '100'
        pp: '15'
      - move: 'Rain Dance'
        source: 'Level 22'
        type: 'Water'
        category: 'Status'
        power: '—'
        accuracy: '∞'
        pp: '5'
      - move: 'Body Slam'
        source: 'Level 29'
        type: 'Normal'
        category: 'Physical'
        power: '85'
        accuracy: '100'
        pp: '15'
      - move: 'Muddy Water'
        source: 'Level 37'
        type: 'Water'
        category: 'Special'
        power: '90'
        accuracy: '85'
        pp: '10'
      - move: 'Recover'
        source: 'Level 46'
        type: 'Normal'
        category: 'Status'
        power: '—'
        accuracy: '∞'
        pp: '10'
    tms:
      - move: 'Attract'
        type: 'Normal'
        category: 'Status'
        power: '—'
        accuracy: '100'
        pp: '15'
      - move: 'Blizzard'
        type: 'Ice'
        category: 'Special'
        power: '110'
        accuracy: '70'
        pp: '5'
      - move: 'Confide'
        type: 'Normal'
        category: 'Status'
        power: '—'
        accuracy: '∞'
        pp: '20'
      - move: 'Dive'
        type: 'Water'
        category: 'Physical'
        power: '80'
        accuracy: '100'
        pp: '10'
      - move: 'Double Team'
        type: 'Normal'
        category: 'Status'
        power: '—'
        accuracy: '∞'
        pp: '15'
      - move: 'Facade'
        type: 'Normal'
        category: 'Physical'
        power: '70'
        accuracy: '100'
        pp: '20'
      - move: 'Frustration'
        type: 'Normal'
        category: 'Physical'
        power: '—'
        accuracy: '100'
        pp: '20'
      - move: 'Hail'
        type: 'Ice'
        category: 'Status'
        power: '—'
        accuracy: '∞'
        pp: '10'
      - move: 'Hidden Power'
        type: 'Normal'
        category: 'Special'
        power: '60'
        accuracy: '100'
        pp: '15'
      - move: 'Ice Beam'
        type: 'Ice'
        category: 'Special'
        power: '90'
        accuracy: '100'
        pp: '10'
      - move: 'Infestation'
        type: 'Bug'
        category: 'Special'
        power: '20'
        accuracy: '100'
        pp: '20'
      - move: 'Protect'
        type: 'Normal'
        category: 'Status'
        power: '—'
        accuracy: '∞'
        pp: '10'
      - move: 'Rain Dance'
        type: 'Water'
        category: 'Status'
        power: '—'
        accuracy: '∞'
        pp: '5'
      - move: 'Rest'
        type: 'Psychic'
        category: 'Status'
        power: '—'
        accuracy: '∞'
        pp: '10'
      - move: 'Return'
        type: 'Normal'
        category: 'Physical'
        power: '—'
        accuracy: '100'
        pp: '20'
      - move: 'Round'
        type: 'Normal'
        category: 'Special'
        power: '60'
        accuracy: '100'
        pp: '15'
      - move: 'Scald'
        type: 'Water'
        category: 'Special'
        power: '80'
        accuracy: '100'
        pp: '15'
      - move: 'Secret Power'
        type: 'Normal'
        category: 'Physical'
        power: '70'
        accuracy: '100'
        pp: '20'
      - move: 'Sleep Talk'
        type: 'Normal'
        category: 'Status'
        power: '—'
        accuracy: '∞'
        pp: '10'
      - move: 'Substitute'
        type: 'Normal'
        category: 'Status'
        power: '—'
        accuracy: '∞'
        pp: '10'
      - move: 'Surf'
        type: 'Water'
        category: 'Special'
        power: '90'
        accuracy: '100'
        pp: '15'
      - move: 'Swagger'
        type: 'Normal'
        category: 'Status'
        power: '—'
        accuracy: '90'
        pp: '15'
      - move: 'Toxic'
        type: 'Poison'
        category: 'Status'
        power: '—'
        accuracy: '90'
        pp: '10'
    tutor_moves:
      - move: 'Earth Power'
        type: 'Ground'
        category: 'Special'
        power: '90'
        accuracy: '100'
        pp: '10'
      - move: 'Icy Wind'
        type: 'Ice'
        category: 'Special'
        power: '55'
        accuracy: '95'
        pp: '15'
      - move: 'Pain Split'
        type: 'Normal'
        category: 'Status'
        power: '—'
        accuracy: '∞'
        pp: '20'
      - move: 'Snore'
        type: 'Normal'
        category: 'Special'
        power: '50'
        accuracy: '100'
        pp: '15'
      - move: 'Water Pulse'
        type: 'Water'
        category: 'Special'
        power: '60'
        accuracy: '100'
        pp: '20'
    egg_moves:
      - move: 'Acid Armor'
        type: 'Poison'
        category: 'Status'
        power: '—'
        accuracy: '∞'
        pp: '20'
      - move: 'Amnesia'
        type: 'Psychic'
        category: 'Status'
        power: '—'
        accuracy: '∞'
        pp: '20'
      - move: 'Brine'
        type: 'Water'
        category: 'Special'
        power: '65'
        accuracy: '100'
        pp: '10'
      - move: 'Clear Smog'
        type: 'Poison'
        category: 'Special'
        power: '50'
        accuracy: '∞'
        pp: '15'
      - move: 'Counter'
        type: 'Fighting'
        category: 'Physical'
        power: '—'
        accuracy: '100'
        pp: '20'
      - move: 'Curse'
        type: 'Ghost'
        category: 'Status'
        power: '—'
        accuracy: '∞'
        pp: '10'
      - move: 'Fissure'
        type: 'Ground'
        category: 'Physical'
        power: '—'
        accuracy: '30'
        pp: '5'
      - move: 'Memento'
        type: 'Dark'
        category: 'Status'
        power: '—'
        accuracy: '100'
        pp: '10'
      - move: 'Mirror Coat'
        type: 'Psychic'
        category: 'Special'
        power: '—'
        accuracy: '100'
        pp: '20'
      - move: 'Mist'
        type: 'Ice'
        category: 'Status'
        power: '—'
        accuracy: '∞'
        pp: '30'
      - move: 'Sludge'
        type: 'Poison'
        category: 'Special'
        power: '65'
        accuracy: '100'
        pp: '20'
      - move: 'Spit Up'
        type: 'Normal'
        category: 'Special'
        power: '—'
        accuracy: '100'
        pp: '10'
      - move: 'Stockpile'
        type: 'Normal'
        category: 'Status'
        power: '—'
        accuracy: '∞'
        pp: '20'
      - move: 'Swallow'
        type: 'Normal'
        category: 'Status'
        power: '—'
        accuracy: '∞'
        pp: '10'
      - move: 'Trump Card'
        type: 'Normal'
        category: 'Special'
        power: '—'
        accuracy: '∞'
        pp: '5'
      - move: 'Yawn'
        type: 'Normal'
        category: 'Status'
        power: '—'
        accuracy: '∞'
        pp: '10'
    evolution_family:
      - species: 'Shellos'
        slug: 'shellos'
        depth: 0
        method: ''
      - species: 'Gastrodon'
        slug: 'gastrodon'
        depth: 1
        method: 'Level 30'
region: ''
anniversary: ''
tier: ''
video: ''
---
